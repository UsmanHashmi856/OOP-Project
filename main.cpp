...
const float PLAYER_SPEED = 6.f, BULLET_SPEED = 20.f, DROP_SPEED = 2.f;
const int FIRE_RATE = 10, INIT_AMMO = 100, MAX_AMMO = 100;
const int  PLAYER_HP = 500, MAX_HP = 500;
const int SCORE_PER_ZOMBIE = 10;
const int HEALTH_DROP_INTERVAL = 1000, HEALTH_DROP_AMOUNT = 100;
const int AMMO_DROP_INTERVAL = 1000, AMMO_DROP_AMOUNT = 30;
...
const int MAX_BULLETS = 100, MAX_ZOMBIES = 50, MAX_DROPS = 20;
...
const int VOLUME_GAMEOVER = 50.f;
const int VOLUME_HEAL = 50.f;
const int VOLUME_AMMO = 50.f;
...
public:
    virtual ~Entity() {}
    virtual void update() {}
    void draw(RenderWindow& window) { if (active) window.draw(sprite); }
    ...
    void setScale(float s) { sprite.setScale(s, s); }
};
...
public:
    ...
    void setup() {
        ...
        bgMusic.play();
        shootSound.setBuffer(shootBuf); shootSound.setVolume(VOLUME_SHOOT);
        zombieSound.setBuffer(zombieBuf); zombieSound.setVolume(VOLUME_ZOMBIE);
        gameOverSound.setBuffer(gameOverBuf); gameOverSound.setVolume(VOLUME_GAMEOVER);
        healSound.setBuffer(healBuf); healSound.setVolume(VOLUME_HEAL);
        ammoSound.setBuffer(ammoBuf); ammoSound.setVolume(VOLUME_AMMO);
    }
...
class UI {
    Font font;
    RectangleShape box, healthBar, fpsBox, menuBox;
    Text text, fpsText;
    const float pad = 10.f, menuPad = 30.f;
public:
    UI() {
        if (!font.loadFromFile("PixelEmulator.ttf")) cout << "Failed to load font\n";
        text.setFont(font);
        text.setCharacterSize(24);
        text.setFillColor(TEXT_COLOR);
        box.setFillColor(BOX_COLOR); box.setOutlineColor(BOX_OUTLINE_COLOR); box.setOutlineThickness(BOX_OUTLINE_THICKNESS);
        healthBar.setFillColor(HEALTHBAR_COLOR);
        fpsText.setFont(font); fpsText.setCharacterSize(20); fpsText.setFillColor(Color::Yellow);
        fpsBox.setFillColor(BOX_COLOR); fpsBox.setOutlineColor(BOX_OUTLINE_COLOR); fpsBox.setOutlineThickness(BOX_OUTLINE_THICKNESS);
        menuBox.setFillColor(BOX_COLOR); menuBox.setOutlineColor(BOX_OUTLINE_COLOR); menuBox.setOutlineThickness(BOX_OUTLINE_THICKNESS);
    }
    ...
    void drawHealthBar(RenderWindow& w, float health) {
        ...
        back.setFillColor(BOX_COLOR); back.setOutlineColor(BOX_OUTLINE_COLOR); back.setOutlineThickness(BOX_OUTLINE_THICKNESS);
    ...
    void drawFPS(RenderWindow& w, float fps) {
        ostringstream ss; ss.precision(2); ss << fixed << "FPS: " << fps;
        ...
        w.draw(fpsBox); fpsText.setPosition(fpsBox.getPosition().x + pad, fpsBox.getPosition().y + pad / 2); w.draw(fpsText);
    }
    void drawTextCentered(RenderWindow& w, const string& s, int size, float x, float y, Color color = TEXT_COLOR) {
        Text t(s, font, size); t.setFillColor(color);
    ...
    void drawMenuBox(RenderWindow& w, const string& s, int size, float x, float y) {
        Text t(s, font, size); t.setFillColor(TEXT_COLOR);
        ...
        t.setOrigin(b.width / 2.f, b.height / 2.f); t.setPosition(x, y);
        w.draw(menuBox); w.draw(t);
    }
};
...
class Game {
    ...
    Drop healthDrops[MAX_DROPS], ammoDrops[MAX_DROPS];
...
public:
    Game() : window(VideoMode(WIDTH, HEIGHT), "Zombie Land") {
        ...
        for (int i = 0; i < MAX_DROPS; i++) { healthDrops[i] = Drop(); ammoDrops[i] = Drop(); }
        if (!soundManager.load()) cout << "Failed to load sounds\n";
        soundManager.setup();
        resetGame();
        ...
        while (window.isOpen()) {
            ...
            if (levelSelect) {
                ...
                if (gameOver) {
                    ...
                    if (!paused) { updateGame(); drawGame(); }
                    else { drawGame(); ui.drawTextCentered(window, "Paused\nPress ESC to resume", 48, WIDTH / 2.f, HEIGHT / 2.f); }
                }
            }
            window.display();
            frameCount++;
...
private:
    void resetGame() {
        ...
        for (int i = 0; i < MAX_DROPS; i++) { healthDrops[i].setActive(false); ammoDrops[i].setActive(false); }
    }
    ...
    void updateGame() {
        ...
        healthDropTimer++;
        ammoDropTimer++;
        for (int i = 0; i < MAX_DROPS; i++) { if (healthDrops[i].isActive()) healthDrops[i].update(); if (ammoDrops[i].isActive()) ammoDrops[i].update(); }
        for (int i = 0; i < MAX_ZOMBIES; i++) {
    ...
    void drawGame() {
        ...
        for (int i = 0; i < MAX_DROPS; i++) { healthDrops[i].draw(window); ammoDrops[i].draw(window); }
        float elapsed;
        if (gameOver) {
            elapsed = timeSurvivedAtGameOver;
        } else {
            elapsed = gameClock.getElapsedTime().asSeconds();
        }
